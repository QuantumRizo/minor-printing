...
export interface VisitingScheduleRow {
  LastName: string;
  Name: string;
  Bunk: string;
  Minor1: string;
...
export interface VisitingParsedData {
  title: string;
  rows: VisitingScheduleRow[];
}

export const parseVisitingCSV = (file: File): Promise<VisitingParsedData> => {
  return new Promise((resolve, reject) => {
    Papa.parse(file, {
      ...
      complete: (results) => {
        ...
        const rows: VisitingScheduleRow[] = data.map((row) => {
          ...
          return {
            ...
            Minor4: truncate(row['Period 4']),
            Minor5: truncate(row['Period 5']),
            Minor6: truncate(row['Period 6']),
          };
        });
